...
6. **Spec review loop** — dispatch spec-document-reviewer subagent (see `spec-document-reviewer-prompt.md`)
7. **Transition to implementation** — invoke `/writing-plans` to create implementation plan
...
- Smaller, well-bounded units are easier to work with — reasoning is better about code that fits in context at once, and edits are more reliable when files are focused
